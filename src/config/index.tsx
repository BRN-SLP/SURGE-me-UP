
import { cookieStorage, createStorage, http } from '@wagmi/core'
import { WagmiAdapter } from '@reown/appkit-adapter-wagmi'
import { mainnet, arbitrum, optimism, base, scroll, polygon, celo } from '@reown/appkit/networks'

// Get projectId from environment or use a placeholder
// NOTE: It is recommended to put this in your .env file as NEXT_PUBLIC_PROJECT_ID
export const projectId = process.env.NEXT_PUBLIC_PROJECT_ID || "b56e18d47c72ab683b10814fe9495694";

if (!projectId) {
    throw new Error("Project ID is not defined")
}

export const networks = [mainnet, arbitrum, optimism, base, scroll, polygon, celo]

// Set up Wagmi Adapter
export const wagmiAdapter = new WagmiAdapter({
    storage: createStorage({
        storage: cookieStorage
    }),
    ssr: true,
    projectId,
    networks
})

export const config = wagmiAdapter.wagmiConfig
